...
// constructor
ErrorChecker::ErrorChecker(string programFileName) :
...
string ErrorChecker::ReplaceAll(std::string str, const std::string& from, const std::string& to) {
    size_t start_pos = 0;
...
void ErrorChecker::parseInst(string command, stringstream &argv, int lineNumber){
	try
	{
		string token;
...
void ErrorChecker::parseVar(stringstream &line, int lineNumber){
	try
	{
		string token;
		vector<char*> arguments;
...
vector<char*> ErrorChecker::parseLine (stringstream &line) {
   string token;
   vector<char*> tokens;
   char* cstr;
...
void ErrorChecker::trimWhitespace(string& str){
...
void ErrorChecker::reportError(string errMsg , int lineNumber /*= -1*/ , bool terminate /*= false*/) {
...
// set parseError
void ErrorChecker::setParseError(bool val) {
	parseError = val;
}

// get a pointer to the map of existing identifiers
map<string,Identifier*> * ErrorChecker::getidentifiers() {
	return & identifiers;
}

// get a pointer to the map of identifiers' prototypes (typeSet)
map<string,Identifier*> * ErrorChecker::getTypes(){
	return & typeSet;
}

// get a pointer to the map of existing labels
map<string, int>* ErrorChecker::getLabels(){
	return & labels;
}

bool ErrorChecker::getErrorStatus(){ return parseError; }
